...
void CPU8080::reset() {
    ...
    std::string welcomeText = "Intel 8080 Emulator.";
    for(char ch : welcomeText) {
        io.getTerminal().printChar(ch);
    }
    io.getTerminal().setCursorPos(0, 2);
}

void CPU8080::push(uint16_t value) {
    memory.write(--regs.SP, (value >> 8) & 0xFF);
    memory.write(--regs.SP, value & 0xFF);
}

uint16_t CPU8080::pop() {
...
void CPU8080::execute(uint8_t opcode) {
    ...
    switch(opcode) {
        ...
        // --- PUSH rp ---
        case 0xC5: push((regs.B << 8) | regs.C); asmInstr = "PUSH B"; break;
        case 0xD5: push((regs.D << 8) | regs.E); asmInstr = "PUSH D"; break;
        case 0xE5: push((regs.H << 8) | regs.L); asmInstr = "PUSH H"; break;
        case 0xF5: push((regs.A << 8) | (regs.Z << 6 | regs.S << 7 | regs.P << 2 | regs.CY)); asmInstr = "PUSH PSW"; break;
        ...
        // --- RET ---
        case 0xC9: regs.PC = pop(); asmInstr = "RET"; break;
        ...
        // --- HLT ---
        case 0x76: halted = true; asmInstr = "HLT"; break;
        ...
        case 0x09:
            {
                uint32_t result = regs.getRegisterPair(_HL) + regs.getRegisterPair(_BC);
        ...
        case 0x19:
            {
                uint32_t result = regs.getRegisterPair(_HL) + regs.getRegisterPair(_DE);
                regs.H = (result >> 8) & 0xFF;
                regs.L = result & 0xFF;
                regs.CY = result > 0XFFFF;
                asmInstr = "DAD D";
        ...
        case 0x29:
            {
                uint32_t result = regs.getRegisterPair(_HL) + regs.getRegisterPair(_HL);
                regs.H = (result >> 8) & 0xFF;
                regs.L = result & 0xFF;
                regs.CY = result > 0xFFFF;
                asmInstr = "DAD H";
        ...
        case 0x39:
            {
                uint32_t result = regs.getRegisterPair(_HL) + regs.SP;
                regs.H = (result >> 8) & 0xFF;
                regs.L = result & 0xFF;
                regs.CY = result > 0xFFFF;
                asmInstr = "DAD SP";
        ...
        // RST 0
        case 0xC7: push(regs.PC); regs.PC = 0x00; asmInstr = "RST 0"; break;
        case 0xCF: push(regs.PC); regs.PC = 0x08; asmInstr = "RST 1"; break;
        case 0xD7: push(regs.PC); regs.PC = 0x10; asmInstr = "RST 2"; break;
        case 0xDF: push(regs.PC); regs.PC = 0x18; asmInstr = "RST 3"; break;
        case 0xE7: push(regs.PC); regs.PC = 0x20; asmInstr = "RST 4"; break;
        case 0xEF: push(regs.PC); regs.PC = 0x28; asmInstr = "RST 5"; break;
        case 0xF7: push(regs.PC); regs.PC = 0x30; asmInstr = "RST 6"; break;
        case 0xFF: push(regs.PC); regs.PC = 0x38; asmInstr = "RST 7"; break;

        // --- RZ ---
        case 0xC8: if(regs.Z) regs.PC = pop(); asmInstr = "RZ"; break;
        case 0xD0: if(!regs.CY) regs.PC = pop(); asmInstr = "RNC"; break;
        case 0xD8: if(regs.CY) regs.PC = pop(); asmInstr = "RC"; break;
        ...
        // --- RPO ---
        case 0xE0: if(!regs.P) regs.PC = pop(); asmInstr = "RPO"; break;

        // --- XTHL ---
        case 0xE3:
            {
                uint16_t temp = regs.getRegisterPair(_HL);
        ...
        // --- RPE ---
        case 0xE8: if(regs.P) regs.PC = pop(); asmInstr = "RPE"; break;

        // --- PCHL ---
        case 0xE9: regs.PC = regs.getRegisterPair(_HL); asmInstr = "PCHL"; break;
        ...
        // --- RP ---
        case 0xF0: if(!regs.S) regs.PC = pop(); asmInstr = "RP"; break;
        ...
        // --- RM ---
        case 0xF8: if(regs.S) regs.PC = pop(); asmInstr = "RM"; break;

        // --- SPHL ---
        case 0xF9: regs.SP = regs.getRegisterPair(_HL); asmInstr = "SPHL"; break;
